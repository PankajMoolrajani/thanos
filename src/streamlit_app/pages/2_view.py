...
from libs.controls import get_component_controls
...
for component in components:
    ...
    for control in controls:
        ...
        with st.form(key=f"control_{control.id}_{component.id}"):
            st.write("---")
            is_enforced = st.checkbox("Is this control enforced?", key=f"enforced_{control.id}_{component.id}")
            details = st.text_area("Implementation details:", key=f"details_{control.id}_{component.id}")
